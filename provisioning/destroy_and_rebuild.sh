...
sudo docker run --name plugindevwp  --net devwp  --ip 172.18.0.3 --env-file ./provisioning/.env-vars --link devmysql:mysql $INSTALL_PATH_OVERRIDE -p 8091:80 -d -v $(pwd)/fullapp:/var/www/html  -v $(pwd):/app leonstafford/wordpress-static-html-plugin
